...
async function main() {
  console.log('=== Sync Thought Embeddings ===');
  ...
  const query1 = `
    ...
    WHERE t.result IN ('WIN', 'LOSE')
      ...
      AND CONCAT(t.session_id, '-', t.symbol) NOT IN (
        SELECT id FROM \`screen-share-459802.magi_analytics.thought_embeddings\`
      )
    LIMIT 30
  `;
  
  const options = { query: query1 };
  ...
  for (const row of rows) {
    try {
      ...
      const insertQuery = `
        INSERT INTO \`screen-share-459802.magi_analytics.thought_embeddings\`
        (id, session_id, symbol, reasoning, llm_provider, confidence, action, trade_result, return_pct, embedding, created_at)
        VALUES (@id, @session_id, @symbol, @reasoning, @llm_provider, @confidence, @action, @trade_result, @return_pct, @embedding, CURRENT_TIMESTAMP())
      `;
